...
textType = ""
mat = ['', '', '', 0]
dic = ["Star", "Square", "Triangle", "Round", "Rien", "Autre"]
dim = [400, 400]
gorobot = True
goraspberry = False
    ...
    def run(self):
        ...
        while True:
            if (len(mat[0]) != 0) or (len(mat[1]) != 0):
                ...
                history_text = Label(textvariable=sv)
                history_text.grid(row=1, column=0)

                sv2.set('Actuel : ' + str(dic[mat[2]]))
                current_text = Label(textvariable=sv2)
                current_text.grid(row=1, column=1)
...
class Robot(Thread):
    ...
    def envoi_position(self, parametre_position):
        self.position_6 = self.raw_data_to_number(self.client.read_input_registers(5, 1).registers[0]) / 1000
        time.sleep(0.1)
        parametre_position[5] = self.position_6
    ...
    def mode_auto(self):
        ...
        while compteur < 6:
            ...
            time.sleep(2)
            resultat = mat[2]
            _global_variable['text'] = _global_variable['text'] + dic[resultat] + ' '
            if resultat == 0:
